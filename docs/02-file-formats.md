...
| `.RSE` ride scripts | ⚠️ | `source/OpenTPW/VM/` | Loader/disassembler restored & tested; **48/106 opcodes** (Batch A pure complete: time/timer/vars/wait). See T-007 + [06-rse-vm-opcodes.md](06-rse-vm-opcodes.md). |
...
- **`VM/Handlers/`**: `Bounce.cs`, `Logic.cs`, `Math.cs`, `Misc.cs`, `Time.cs`, `Hierarchy.cs`
  → **48 / 106 handlers** (loader + disassembler restored; LIFO call stack;
  branches/JSR/RETURN/END; date/time + timers; child/parent vars; WAIT scheduler).
- **Known limitation**: `BranchTo` is still "hacky" (offsets converted by hand).

**Remaining VM work**: 58 opcodes unimplemented. The `pure` batch is essentially done (only
`HUSH`/`HOP` left, semantics unread); the **63 `engine` opcodes** are blocked on the ride
engine (objects, animations, sound, lights, walk/limbo) and on `SPAWNCHILD`. Wire the VM to
real rides. See T-007.
