...
| [T-045](T-045-coaster-track-editor.md) | 🟡 Feature | ⚠️ Slices 1–3a | Coaster: station + track-laying tool closing into a loop at the `<` entry, rendered as a smooth Catmull-Rom **track ribbon** (real Trak_sec texture, on pylons), with a train of real CrocCar.MD2 cars gliding it and carrying visible riders (occupancy-driven) — done, verified; real peep boarding/scream + .hmp rail profile remain (slice 3b) |
