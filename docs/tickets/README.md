...
| [T-050](T-050-peep-simulation-depth.md) | 🟡 Feature | ⚠️ Mostly done | **Ride ratings & thoughts** (`Peep.RateRide` → satisfaction + `RideThought`, feeding mood / `Ride.Rating` reputation / rating-weighted ride choice) **+ water-aware pathfinding** (`PlacementGrid` water layer — impassable & unbuildable; `MarkWaterFromTerrain`; A* routes around lakes) — both unit-tested. Only the real gate node remains |
