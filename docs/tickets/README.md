...
| [T-041](T-041-ride-shop-placement.md) | 🟡 Feature | ⚠️ Core done | Ride & shop placement + a **clickable build/manage UI** (`HudPanel` base; `BuildPanel` catalog — all items mouse-selectable, fixes the >9 cap; `ManagePanel` — fee/loan/price/research buttons; blocks world-clicks over panels) + **lobby↔in-park HUD split** (`Level.InPark`) + **sell/demolish** rides **and shops** (SELL button / Delete → full teardown incl. coaster track/train, frees grid cells, 50% refund); footprint preview + cost charging + queues; only ride rotation remains |
