...
| [T-050](T-050-peep-simulation-depth.md) | 🟡 Feature | ⚠️ Partial | **Ride ratings & thoughts done**: `Peep.RateRide` (excitement × value-for-money × reliability → satisfaction + `RideThought`) drives peep mood, the ride's running `Ride.Rating` (reputation), and ride choice (weighted by rating) — a pricing/reliability feedback loop, unit-tested. Remaining: water-aware pathfinding + real gate node |
