...
| [T-048](T-048-ride-node-geometry-movement.md) | 🟡 Feature | ⚠️ Partial | Ride model **node graph decoded** (count@0x48, table@0x7c, 0x14B/node = `{typeMask, nodeId, …}`; resolver `FUN_0044b220` matches `type&selector && id`) — exposed in `ModelFile.Nodes`, unit-tested, confirmed on real models (Bird=11/gokarts=3/coaster1=9). Remaining: node **positions** bind to bone transforms at runtime (deeper decode), then the consumers (car paths, WALKON/ADDHEAD placement, T-047 positions) |
