...
| [T-041](T-041-ride-shop-placement.md) | 🟡 Feature | ⚠️ Core done | Ride & shop placement: catalog + footprint preview + place-on-click with cost charging + queue registration, now with a **clickable catalog UI** (`BuildPanel` — all items mouse-selectable, fixes the >9 number-key cap; blocks world-clicks over the panel); rotation/sell remain |
