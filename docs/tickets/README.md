...
| [T-041](T-041-ride-shop-placement.md) | 🟡 Feature | ⚠️ Core done | Ride & shop placement + a **clickable build/manage UI** (`HudPanel` base; `BuildPanel` catalog — all items mouse-selectable, fixes the >9 cap; `ManagePanel` — fee/loan/price/research buttons; blocks world-clicks over panels); footprint preview + cost charging + queues; rotation/sell remain |
