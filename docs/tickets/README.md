...
| [T-045](T-045-coaster-track-editor.md) | 🟡 Feature | ⚠️ Slices 1–3a | Coaster: station placeable/queues + track-laying tool (elevated segments+pylons) that closes into a loop at the `<` entry + a train of real CrocCar.MD2 cars (shuttle when open, continuous loop when closed) — done, verified; 3D curved track + peep boarding/scream remain (slice 3b) |
