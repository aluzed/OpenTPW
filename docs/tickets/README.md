...
| [T-037](T-037-ride-cycle-sound.md) | 🟡 Feature | ⚠️ Bugs fixed | Ride SFX: SPAWNSOUND string-resolved (no spurious clip) + ADDOBJ "explosion" spam (Crunch via param -1) suppressed; screams correct; duration done. Correct per-cue SFX needs the EVENT/EventMap binding |
...
| [T-045](T-045-coaster-track-editor.md) | 🟡 Feature | ⚠️ Slices 1–3a | Coaster: station + track-laying tool closing into a loop at the `<` entry, rendered as a smooth Catmull-Rom **track ribbon** (real Trak_sec texture, on pylons), with a train of real CrocCar.MD2 cars (animated via CrocCarM1..3 frames) gliding it and carrying visible riders (occupancy-driven) — done, verified; real peep boarding/scream + .hmp rail profile remain (slice 3b) |
